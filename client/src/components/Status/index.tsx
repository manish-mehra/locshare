...
function index({locationStatus, socketStatus}: StatusProps) {
  ...
  return (
    <section className='flex flex-wrap gap-2'>
      {
        locationStatus && (
          <div className={`p-2 rounded-full ${locationStatus === 'accessed' ? 'bg-green-500' : 'bg-red-500'}`}>
      ...
      {
        socketStatus && (
          <div className={`p-2 rounded-full ${socketStatus === 'connected' ? 'bg-green-500' : 'bg-red-500'}`}>
